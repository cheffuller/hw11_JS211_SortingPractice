...
const atxIdx = nums.findIndex((el, idx) => el + idx === 512)
...
const findWSNs = (arr) => {
    newArray = [];
    for (const el of arr) {
        newArray.push(el.weather_state_name)
    }
    return newArray
}

const filterArray = (arr, fn) => {
    const arrayToFilter = fn(arr);
    const outputArray = arrayToFilter.filter(function (v, i, self) {
        return i == self.indexOf(v);
    });
    return outputArray;
}

const weatherStates = filterArray(weather, findWSNs)
...
const findId = (arr) => {
    for (const el of arr) {
        if (el.min_temp = 15.915) {
            return el.id
        }
    }
}

const idealTemp = findId(weather)
